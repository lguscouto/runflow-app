...
export function VO2MaxFitnessCard({ estimate }: VO2MaxFitnessCardProps) {
  ...
  const classificationColor = getFitnessForegroundVar(classification.category);
  const classificationBadgeColor = getFitnessBadgeBackgroundVar(classification.category);
  ...
  return (
    <div className="stat-card relative overflow-hidden bg-gradient-to-br from-[var(--color-surface-fitness-start)] via-[var(--color-surface-fitness-middle)] to-[var(--color-surface-fitness-middle)] border border-[var(--border)] rounded-2xl p-5 md:p-6 shadow-xl">
      {/* Background glowing ambient light */}
      <div
        className="absolute -top-12 -right-12 w-44 h-44 rounded-full blur-3xl opacity-20 pointer-events-none"
        style={{ backgroundColor: classificationBadgeColor }}
      />
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2.5">
          <span
            className="w-10 h-10 rounded-xl flex items-center justify-center text-[var(--color-fitness-badge-foreground)] shrink-0 shadow-md"
            style={{ backgroundColor: classificationBadgeColor }}
          >
      ...
      {showInfo && (
        <div className="mb-4 p-3 bg-[var(--surface)] border border-[var(--border)] rounded-xl text-xs text-[var(--muted)] animate-fadeIn">
          <p className="leading-relaxed">{t("vo2max.what_is_it")}</p>
        </div>
      )}

      {/* Main Metric & Fitness Badge */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-center bg-[var(--surface)] border border-[var(--border)] rounded-xl p-4 mb-4">
        <div>
          <span className="text-xs font-semibold uppercase tracking-wider text-[var(--muted)]">
            {t("vo2max.estimated_label")}
          </span>
          <div className="flex items-baseline gap-2 mt-1">
            <span className="text-4xl font-extrabold tracking-tight text-[var(--text)] font-mono">
          ...
          <div className="mt-2 inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-bold"
               style={{
                 backgroundColor: classificationBadgeColor,
        ...
        <div className="sm:border-l sm:border-[var(--border)] sm:pl-4">
          ...
          <div className="flex items-baseline gap-2 mt-1">
            <span className="text-3xl font-extrabold text-[var(--color-status-warning)] font-mono">
              {t("vo2max.fitness_age_years", { age: estimate.fitnessAge })}
            </span>
          </div>
          <p className="text-xs text-[var(--muted)] mt-1">
            {ageDiff !== null && ageDiff > 0 ? (
              <span className="text-[var(--color-status-positive)] font-medium">
      ...
      <div className="space-y-1.5 mb-4">
        <div className="flex justify-between text-[10px] text-[var(--muted)] font-medium px-0.5">
          <span>{t("vo2max.gauge_beginner")} (30)</span>
          <span>{t("vo2max.gauge_fair")} (38)</span>
          <span>{t("vo2max.gauge_good")} (45)</span>
          <span>{t("vo2max.gauge_excellent")} (52)</span>
          <span>{t("vo2max.gauge_superior")} (60+)</span>
        </div>
        <div className="relative h-2.5 w-full bg-[var(--surface-hover)] rounded-full overflow-hidden">
          {/* Gradient bar */}
        ...
        <div className="relative h-2 w-full">
          ...
          >
            <div className="w-4 h-4 rounded-full border-2 border-[var(--text)] bg-[var(--surface)] shadow-md flex items-center justify-center">
              <div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: classificationBadgeColor }} />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-[var(--border)] text-[11px] text-[var(--muted)]">
        <div className="flex items-center gap-1.5">
          <span className="w-2 h-2 rounded-full" style={{ backgroundColor: classificationBadgeColor }} />
          <span>{methodLabel}</span>
          {estimate.sampleCount > 0 && (
            <span>• {t("vo2max.samples_count", { count: estimate.sampleCount })}</span>
          )}
        </div>
        <span className="bg-[var(--surface-hover)] px-2 py-0.5 rounded text-[10px] font-medium text-[var(--text)]">
          {confidenceLabel}
        </span>
      </div>
    </div>
